...
function bundledHeader(label) {
  return `/* Profuzion ${label} — WordPress: Google Fonts + v2.css tokens (BEM .pfz). Body classes: pfz pfz-v6. */
...
const flatHeader = `/* Profuzion v6 — static CSS mirror of src/app/v2/v2.css (BEM .pfz). Load fonts via theme or @import. */

`;

const bundledV6 = bundledHeader("v6");
const bundledV5 = bundledHeader("v5");
const bundledV2 = bundledHeader("v2");

const outputs = [
  ["tools/wordpress/profuzion-brick-child/assets/css/profuzion-v6-wp-bundled.css", bundledV6 + withFonts],
  ["tools/wordpress/profuzion-v6-wp-bundled.css", bundledV6 + withFonts],
  ["tools/wordpress/profuzion-v6.css", flatHeader + withFonts],
  ...
  ["tools/wordpress/profuzion-v5.css", flatHeader.replace("v6", "v5") + withFonts],
  ...
  ["tools/wordpress/profuzion-v2.css", flatHeader.replace("v6", "v2") + withFonts],
];

for (const [p, data] of outputs) {
  fs.writeFileSync(p, data);
}
console.log("wrote", outputs.length, "CSS mirrors (v6 primary + v5/v2 paths kept in sync)");

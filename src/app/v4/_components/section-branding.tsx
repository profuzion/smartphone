...
/**
 * Profuzion · v4 — Branding case studies (§3).
 *
 * Layout
 *   ┌──────────────────────────────────────────────────────────────┐
 *   │  // brand systems                                            │
...
export function SectionBranding() {
  ...
  useGSAP(
    () => {
      ...
      // Each spread fades up on first reveal
      const spreads = gsap.utils.toArray<HTMLElement>("[data-brand-spread]");
      spreads.forEach((el) => {
        gsap.from(el.querySelector("[data-brand-spread-inner]"), {
          opacity: 0,
          y: 40,
          duration: 0.9,
          ease: "expo.out",
        ...
        // Counter-parallax: monogram drifts down, text column drifts up.
        // The two columns slide past each other as you scroll the spread.
        const mono = el.querySelector<HTMLElement>("[data-brand-mono]");
        const details = el.querySelector<HTMLElement>("[data-brand-details]");
        if (mono) {
          gsap.fromTo(
            mono,
            { y: -40 },
      ...
      // Stacking-deck depth — previous spread recedes as next one covers it
  ...
  return (
    <section
      ref={root}
      id="branding"
      className="p-section--ink relative isolate"
    >
...
function BrandSpread({
  ...
  return (
    <article
      data-brand-spread-inner
      className="v4-card v4-card--paper relative overflow-hidden"
    >
      {/* Pin header — peeks above the next card in the deck */}
      <header className="v4-card__pin relative">
        <span className="v4-card__pin-num">
          case · {String(index).padStart(2, "0")}
        </span>
        <span className="v4-card__pin-name">{brand.client}</span>
        <span className="v4-card__pin-meta">
          {brand.industry} · {brand.year}
        </span>
      </header>

      {/* Spread body — alternating left/right */}
      <div
        className={
          "grid items-center gap-10 px-6 py-10 sm:px-10 sm:py-14 lg:grid-cols-12 lg:gap-14 lg:px-12 lg:py-16 " +
          (flip ? "lg:[direction:rtl]" : "")
        }
      >
        {/* Monogram column */}
        <div className="relative lg:col-span-7 lg:[direction:ltr]">
          <BrandMonogram brand={brand} />
        </div>

        {/* Details column */}
        <div data-brand-details className="lg:col-span-5 lg:[direction:ltr]">
